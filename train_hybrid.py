...
if __name__ == "__main__":
    ...
    cfg = NeuralCDEConfig(
        ...
        func_mlp_hidden=16,
        dec_rho_hidden=8,
        dec_p=4,              # neural basis dimension (small)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        # Accumulate GLS sufficient statistics across all batches
        ...
        for batch in loader:
            ...
            mu, V, AtA_batch, Atb_batch = model(t_pad, x_pad, c_mask, s, mask, y_pad=y_pad)
        ...
        if avg < best_loss:
            ...
            torch.save(
                ...
                "checkpoints/simuS2a_model_step2.pt",
            )
    ...
    print(f"\n  D matrix ({D.shape[0]}x{D.shape[1]}):")
    ...
    # Use full dataset (no shuffle) for stable PDP
    eval_loader = DataLoader(dataset, batch_size=256, shuffle=False, collate_fn=collate_pad)

    bmi_values = [20, 23, 26, 29, 32, 35]
    results, ages, masks, times = compute_pdp(model, eval_loader, device, bmi_values, n_tv=n_tv)

    plot_pdp(results, ages, masks, times, bmi_values, save_path="pdp_bmi_by_age.png")
    plot_pdp_marginal(results, masks, times, bmi_values, save_path="pdp_bmi_marginal.png")
