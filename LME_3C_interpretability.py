...
def permute_bmi_keep_length_truncate_or_keep(
    ...
    d["BMI_perm"] = pd.to_numeric(d[bmi_col], errors="coerce").to_numpy(copy=True)
    ...
    for rid in ids:
        ...
        # donor BMI points
        don_pts = don_sorted[[time_col, bmi_col]]
        if don_pts.shape[0] == 0:
            continue

        td = don_pts[time_col].to_numpy()   # (K,)
        yd = pd.to_numeric(don_pts[bmi_col], errors="coerce").to_numpy(dtype=float)
        K = td.shape[0]
        ...
        for k in range(R):
            ...
            d.at[row_idx, bmi_col+"_perm"] = yd[j]
            d.at[row_idx, bmi_col+"_donor_id"] = did
            d.at[row_idx, bmi_col+"_donor_time"] = td[j]
...
permuted_df, _, _ = permute_bmi_keep_length_truncate_or_keep(df, bmi_col='GLUC')
...
permuted_df, _, _ = permute_bmi_keep_length_truncate_or_keep(df, bmi_col='HDL')
...
permuted_df, _, _ = permute_bmi_keep_length_truncate_or_keep(df, bmi_col='PAD')
