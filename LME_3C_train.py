...
if __name__ == "__main__":
    ...
    train_data = process_data(train_df, id_col, features, static_features, target_col, 
                              with_only_static_features=with_only_static_features, scaler=None, metabolic_baseline=metabolic_features_baselines)
    val_data = process_data(val_df, id_col, features, static_features, target_col, 
                            with_only_static_features=with_only_static_features, scaler=None, metabolic_baseline=metabolic_features_baselines)
    ...
    patience_counter = 0
    model_save_path = 'EXPs/latent_4_ODE_diagoG_decodergru.pth'
    # ----------------------------------------------------------------
    ...
    for epoch in range(EPOCHS):
        ...
        for batch in train_loader:
            ...
            if not with_only_static_features:
                ...
                t, y, s_i, mask, subject_id, metabolic_baseline_data = [d.to(device) for d in batch.values()]
                predicted_mean, V = model(s_i, t, metabolic_baseline=metabolic_baseline_data)
        ...
        with torch.no_grad():
            for batch in val_loader:
                if not with_only_static_features:
                    ...
                    t, y, s_i, mask, patient_id, metabolic_baseline_data = [d.to(device) for d in batch.values()]
                    predicted_mean, V = model(s_i, t, metabolic_baseline=metabolic_baseline_data)
