"""
Model selection experiment for S2 (cumulative BMI burden).

Saves checkpoints for post-hoc dPDP bias analysis.

Usage:
    python train_ODE_model_selection_S2.py --n_sims 10 --data_dir simu_datasets/S5_sims
"""
...
def get_s2_configs() -> list[ExpConfig]:
    return [
        # ExpConfig(name="A_ode_only", use_bmi_skip=False, use_bmi_in_ode=True,
        #           reg_mode=None, lambda_reg=0.0),
        ...
        #           reg_mode="group_lasso", lambda_reg=0.1),
        ExpConfig(name="D_noskip", use_bmi_skip=False, use_bmi_in_ode=True,
                  reg_mode=None, skip_statics=False, lambda_reg=0),
    ]
...
def build_model(exp, bmi_mean, bmi_std, device):
    cfg = NeuralODEConfig(
        ...
        depth=2, dropout=0.0, euler_steps_per_interval=4, ode_solver="rk4",
    )
...
def train_one(model, train_loader, test_loader, device,
              ...
              max_epochs=1000, patience=150, print_every=50,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str, default="results_simu/model_selection_S2_2.json")
    parser.add_argument("--ckpt_dir", type=str, default="checkpoints/model_selection_S2")
    ...
    print(f"\n{'='*70}")
    print(f"MODEL SELECTION — S2 ({args.n_sims} replicates)")
    print(f"{'='*70}")
