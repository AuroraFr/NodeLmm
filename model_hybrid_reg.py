...
class Decoder(nn.Module):
    def __init__(self, latent_dim, p, q=3,
        ...
        self.include_fe_splines = include_fe_splines
        ...
        # --- Parametric beta dimension ---
        fe_dim = self.fe_spline_df if self.include_fe_splines else 0
        self.n_W = 1 + fe_dim + n_tv + n_static + n_interactions
        ...
        if use_rho_net:
            # MLP: z(t) → R^p → weighted sum → scalar
    ...
    def _build_W(self, t_pad, x_pad, static):
        ...
        # W = [1, (optional: ns1..nsK), tv_covs, static, (interactions)]
        parts = [ones]

        # Fixed-effect time spline basis (optional)
        if self.include_fe_splines:
            if self.precomputed_splines:
                fe_spline_cols = x_pad[:, :, self.n_tv : self.n_tv + self.fe_spline_df]
            else:
                knots = self.fe_spline_knots.cpu().numpy()
                boundary = self.fe_spline_boundary.cpu().numpy()
                fe_spline_cols = natural_spline_basis(t_pad, knots, boundary)
            parts.append(fe_spline_cols)

        parts.extend([tv_covs, static_exp])
    ...
    def forward(self, z_t, t_pad, x_pad, static, y_pad=None, obs_mask=None,
        ...
        if self.use_rho_net:
            ...
            h = (z_t * self.w_neural).sum(dim=-1)                  # (N, T)
...
class NeuralCDEModel(nn.Module):
    ...
    def __init__(self, x_dim: int, static_dim: int = 0, cfg: NeuralCDEConfig = NeuralCDEConfig(),
        ...
        self.decoder = Decoder(
            ...
            include_fe_splines=include_fe_splines,
