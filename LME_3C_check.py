...
with_only_static_features = True
...
plot_mean_predictions(train_df, model="ODE", mode="fit", 
                      prediction_file="results/ODE_3C_fit.npy", hlme_prediction_file="results/ISA15_Model_4_train_fit.csv")
...
print(filter_patient_with_id(10621, train_dataset))

# train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=True, collate_fn=collate_fn)
# val_loader = DataLoader(val_dataset, batch_size=BATCH_SIZE, shuffle=False, collate_fn=collate_fn)

# if with_only_static_features and metabolic_features_baselines:
#     ode_dynamics = ODEFunc(hidden_dim=LATENT_DIM, static_dim=static_features_dim + len(ODE_features))
#     scripted_ode_func = torch.jit.script(ode_dynamics)
#     model = ODENet(scripted_ode_func, static_features_dim+len(ODE_features), LATENT_DIM, device, fullG=False, GRU=False).to(device)
#     model.load_state_dict(torch.load('EXPs/latent_4_ODE_diagoG_nostaticinode.pth')['model_state_dict'])
# else:
#     model = CDEModel(len(features)* 2 + 1, static_features_dim, LATENT_DIM, device, GRU=True).to(device)
...
# np.save("results/ODE_3C_pred.npy", predicitons)
...
# hlme_predictions = pd.read_csv('results/ISA15_Model_4_train_fit.csv', sep=',')
...
# plt.savefig("figures/ODE_train_blup_predictions_fit_mode.pdf", format='pdf', bbox_inches='tight')
# plt.close() 
...
# plt.savefig("figures/ODE_train_blup_predictions_pred_mode.pdf", format='pdf', bbox_inches='tight')
# plt.close()
