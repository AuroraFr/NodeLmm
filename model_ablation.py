...
class Decoder(nn.Module):
    def __init__(self, latent_dim, p, q=3,
        ...
        if precomputed_splines:
            ...
            self.register_buffer('fe_spline_boundary',
                                 torch.tensor(np.asarray(fe_spline_boundary if fe_spline_boundary is not None else [0, 1]), dtype=torch.float32))
            ...
            self.register_buffer('re_spline_boundary',
                                 torch.tensor(np.asarray(re_spline_boundary if re_spline_boundary is not None else [0, 1]), dtype=torch.float32))
            # Verify q matches
        ...
        # --- Parametric beta: [intercept, ns1..nsK, tv_covs, static_covs, interactions] ---
        #   dim = 1 + fe_spline_df + n_tv + n_static + n_interactions
        self.n_W = 1 + self.fe_spline_df + n_tv + n_static + n_interactions
        self.beta = nn.Parameter(torch.zeros(self.n_W))
    ...
    def _orthogonal_project(self, h, W, obs_mask):
        """
        Project neural output h onto the orthogonal complement of col(W),
        using only observed (non-padded) positions.

        h:        (N, T)    scalar neural output
        W:        (N, T, p) parametric design matrix
        obs_mask: (N, T)    binary mask (1=observed, 0=padded)
        Returns h_perp: (N, T)
        """
        N, T = h.shape
        h_flat = h.reshape(-1)                        # (N*T,)
        W_flat = W.reshape(-1, W.shape[-1])           # (N*T, p)
        obs = obs_mask.reshape(-1).bool()             # (N*T,)

        h_obs = h_flat[obs]                           # (M,)
        W_obs = W_flat[obs]                           # (M, p)

        # QR for numerical stability
        Q, _ = torch.linalg.qr(W_obs)                # Q: (M, p)
        h_parallel = Q @ (Q.t() @ h_obs)             # projection onto col(W)
        h_perp_obs = h_obs - h_parallel

        # Scatter back
        h_perp = torch.zeros_like(h_flat)
        h_perp[obs] = h_perp_obs

        return h_perp.reshape(N, T)

    def forward(self, z_t, t_pad, x_pad, static, obs_mask=None, return_components=False):
        N, T, H = z_t.shape
        device, dtype = z_t.device, z_t.dtype

        # 1. Parametric design matrix: [1, ns1(t)..ns3(t), BMI, GLUC, AGE, SEX, DIPNIV]
        W = self._build_W(t_pad, x_pad, static)            # (N, T, n_W)
        mu_parametric = (W * self.beta).sum(dim=-1)         # (N, T)

        # 2. Neural flexible fixed effects
        rho = self.rho_net(z_t)
        rho = self.rho_norm(rho)
        mu_neural = (rho * self.beta_neural).sum(dim=-1)    # (N, T)

        # 3. Orthogonal projection: force h ⊥ col(W)
        #    This ensures beta is estimated as in classical HLME:
        #    β_BMI is the BMI main effect *conditional on the time splines*
        if obs_mask is not None:
            mu_neural = self._orthogonal_project(mu_neural, W, obs_mask)

        # 4. Total mean
        mu = mu_parametric + mu_neural

        # 5. Random effects on FIXED spline basis of time
        Z = self._build_Z(t_pad, x_pad)                       # (N, T, q)

        # Zero out Z at padded positions.
        # V_re = Z @ D @ Z^T — padded rows would contribute spurious
        # covariance entries. masked_NLL extracts only the observed
        # submatrix, so this is safe either way, but zeroing out padded
        # rows keeps V_re clean and better conditioned.
        if obs_mask is not None:
            Z = Z * obs_mask.unsqueeze(-1)                  # (N, T, q) * (N, T, 1)
        ...
        if return_components:
            return mu, V, mu_parametric, mu_neural, Z, D, sig2
        return mu, V
...
class NeuralCDEModel(nn.Module):
    ...
    def forward(
        ...
        X_in = torch.cat([t_pad[..., None], x_pad, masks[..., None]], dim=-1)
        ...
        grid = torch.arange(T, device=device, dtype=dtype) #to be defined
        ...
        x0 = X.evaluate(grid[0])  # (N, input_channels)
        encoder_in = torch.cat([x0[:,:-1], static_covariates], dim=-1)
        ...
        return self.decoder(zt, t_pad, x_pad, static_covariates, obs_mask=obs_mask) # (N, T)